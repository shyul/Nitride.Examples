...
/// Copyright 2001-2008, 2.014-2018 Xu Li - me@xuli.us
/// 
...
namespace Nitride
{
    ...
    {
        public static void ValueGenerator(IDictionary<string, double> res, ICollection<double> decVal, Range<double> range, int shift = 2, char pt = 'R')
        {
            double ptThre = Math.Pow(10, shift);
            string fm = "0.";
            for (int i = 0; i < shift; i++) fm += "0";

            for (int i = 0; ; i++)
            {
                double m = Math.Pow(10, i);

                foreach (double b in decVal)
                {
                    double value = b * m;
                    ...
                    if (range.Contains(value))
                    {
                        ...
                        if (value < ptThre)
                        {
                            valueCode = value.ToString(fm).Replace('.', pt).Substring(0, shift + 2);
                        }
                        else
                        {
                            valueCode = (b * ptThre).ToString().Substring(0, shift + 1) + (i - shift).ToString();
                        }
                        res.Add(valueCode, value);
                    }
        ...
        public static readonly HashSet<double> E12Values = new HashSet<double>() {  1.00, 1.20, 1.50, 1.80, 2.20, 2.70,
                                                                                    3.30, 3.90, 4.70, 5.60, 6.80, 8.20 };

        public static readonly HashSet<double> E24Values = new HashSet<double>() {  1.00, 1.10, 1.20, 1.30, 1.50, 1.60,
                                                                                    1.80, 2.00, 2.20, 2.40, 2.70, 3.00,
                                                                                    3.30, 3.60, 3.90, 4.30, 4.70, 5.10,
                                                                                    5.60, 6.20, 6.80, 7.50, 8.20, 9.10 };

        public static readonly HashSet<double> E96Values = new HashSet<double>() {  1.00, 1.02, 1.05, 1.07, 1.10, 1.13,
        ...
        public static readonly HashSet<double> E192Values = new HashSet<double>() { 1.00, 1.01, 1.02, 1.04, 1.05, 1.06,
