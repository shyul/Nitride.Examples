...
namespace Nitride.EE.FTDI
{
    ...
    public class FTDI
    {
        ...
        public FT_STATUS SyncMPSSEInterface(byte badCommand = 0xAB)
        {
            ...
            if (cnt != 0)
            {
                Console.WriteLine("Error - 0x84 MPSSE receive buffer should be empty: " + status + ", Count = " + cnt);
            ...
            if (cnt != 0)
            {
                Console.WriteLine("Error - 0x85 MPSSE receive buffer should be empty: " + status + ", Count = " + cnt);
                status |= SetBitMode(0x00, 0x00); // Reset controller
                Close();
                return status;
            }
        ...
        {
            uint NumBytesSent = 0;
            var status = Write(data, data.Length, ref NumBytesSent);
            if (status == FT_STATUS.FT_OK)
            {
                if (NumBytesSent == data.Length)
        ...
        public FT_STATUS MPSSE_Init_SPI()
        {
            ...
            status |= SetTimeouts(0, 5000); // Sets the read and write timeouts in milliseconds  
            ...
            status |= SetBitMode(0x0, 0x0); // Reset controller
            status |= SetBitMode(0x0, 0x2); // Enable MPSSE mode
            Thread.Sleep(50);
