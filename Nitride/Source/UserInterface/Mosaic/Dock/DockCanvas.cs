...
namespace Nitride
{
    ...
    public class DockCanvas : UserControl
    {
        ...
        private static void DockHelper_Paint(object sender, PaintEventArgs e)
        {
            ...
            if (NextContainer != null && !(NextContainer == ActiveDockContainer && ActiveDockContainer.Count < 2))
            {
                if (NextContainer.Dock_C != Rectangle.Empty) g.DrawImage(Nitride.Properties.Resources.Dock_Center, NextContainer.Dock_C);
                if (NextContainer.Dock_T != Rectangle.Empty) g.DrawImage(Nitride.Properties.Resources.Dock_Top, NextContainer.Dock_T);
                if (NextContainer.Dock_B != Rectangle.Empty) g.DrawImage(Nitride.Properties.Resources.Dock_Bottom, NextContainer.Dock_B);
                if (NextContainer.Dock_L != Rectangle.Empty) g.DrawImage(Nitride.Properties.Resources.Dock_Left, NextContainer.Dock_L);
                if (NextContainer.Dock_R != Rectangle.Empty) g.DrawImage(Nitride.Properties.Resources.Dock_Right, NextContainer.Dock_R);
            }
        }
        #endregion
