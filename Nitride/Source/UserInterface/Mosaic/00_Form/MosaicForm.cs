...
namespace Nitride
{
    ...
    public class MosaicForm : Form
    {
            ...
            else
            {
                switch (m.Msg)
                {
                    ...
                    case (WindowsMessages.CREATE):
                        {
                            RECT rcClient = new();
                            User32.GetWindowRect(this.Handle, ref rcClient);
                            User32.SetWindowPos(this.Handle, IntPtr.Zero,
                                         rcClient.Left, rcClient.Top,
                                         rcClient.Right - rcClient.Left, rcClient.Bottom - rcClient.Top,
                                         SWP.FRAMECHANGED);
                            m.Result = Nitride.GUI.MSG_HANDLED;
                            base.WndProc(ref m);
                            break;
                        }
        ...
        private IntPtr HitTestNCA(IntPtr hWnd)
        {
            ...
            RECT rcWindow = new();
            Nitride.WindowsNativeMethods.User32.GetWindowRect(hWnd, ref rcWindow); // this.Handle

            // Get the frame rectangle, adjusted for the style without a caption.
            RECT rcFrame = new();
            Nitride.WindowsNativeMethods.User32.AdjustWindowRectEx(ref rcFrame, WindowStyles.OVERLAPPEDWINDOW & ~WindowStyles.CAPTION, false, 0); // The last is supposed to be NULL
